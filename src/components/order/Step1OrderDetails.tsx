  ...
  const updateItemQuantity = (id: string, newQuantity: number) => {
    ...
    const updatedItems = formData.items.map(item => {
      if (item.id === id) {
        ...
        return { ...item, quantity: newQuantity, ...pricing };
      }
      return item;
    });
              ...
              <div className="mt-3 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                {chickenType && (
                  <>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="space-y-2">
                        {Object.entries(pricingConfig.chickenCategories).map(([weightRange, price]) => {
                          ...
                          return (
                            <div key={weightRange} className={`flex items-center justify-between p-3 rounded-lg border transition-all duration-200 ${quantity > 0 ? 'border-green-300 bg-green-50' : 'border-gray-200 bg-white'
                              }`}>
                    ...
                    {/* Cut-up Options (only for ready-to-cook) */}
                    {chickenType === 'ready-to-cook' && (
                      <div className="mb-4">
                        <label className="block text-sm font-medium text-gray-700 mb-2">Cut Up Option</label>
                        <div className="space-y-2">
                          <button
                            type="button"
                            onClick={() => {
                              const updatedItems = formData.items.map(item => {
                                if (item.productType === 'chicken' && item.chickenNature === 'ready-to-cook') {
                                  return { ...item, cutUp: 'no', cutPieces: '' };
                                }
                                return item;
                              });
                              onChange({ ...formData, items: updatedItems });
                            }}
                            className={`w-full p-3 rounded-lg border-2 text-left transition-colors ${formData.items.some(item => item.productType === 'chicken' && item.chickenNature === 'ready-to-cook' && item.cutUp === 'no')
                              ? 'border-gray-300 bg-white text-gray-700'
                              : 'border-green-500 bg-green-50 text-green-700'
                              }`}
                          >
                            <div className="font-medium">No Cut Up - Whole Chicken</div>
                            <div className="text-xs text-gray-500">Receive whole chicken</div>
                          </button>
                          ...
                          <button
                            ...
                            onClick={() => {
                              const updatedItems = formData.items.map(item => {
                                if (item.productType === 'chicken' && item.chickenNature === 'ready-to-cook') {
                                  return { ...item, cutUp: 'yes', cutPieces: '6' };
                                }
                                return item;
                              });
                              onChange({ ...formData, items: updatedItems });
                            }}
                            className={`w-full p-3 rounded-lg border-2 text-left transition-colors ${formData.items.some(item => item.productType === 'chicken' && item.chickenNature === 'ready-to-cook' && item.cutUp === 'yes')
                              ? 'border-green-500 bg-green-50 text-green-700'
                              : 'border-gray-300 bg-white text-gray-700'
                              }`}
                          >
                            <div className="font-medium">Cut Up - Pieces</div>
                            <div className="text-xs text-gray-500">Cut into pieces (+{pricingConfig.cutUpFeeLow} CFA per chicken)</div>
                          </button>
                        </div>

                        {/* Number of pieces (if cut up is selected) */}
                        {formData.items.some(item => item.productType === 'chicken' && item.chickenNature === 'ready-to-cook' && item.cutUp === 'yes') && (
                          <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">Number of Pieces per Chicken</label>
                            <input
                              type="number"
                              min="2"
                              max="20"
                              value={formData.items.find(item => item.productType === 'chicken' && item.chickenNature === 'ready-to-cook' && item.cutUp === 'yes')?.cutPieces || '6'}
                              onChange={(e) => {
                                const pieces = e.target.value;
                                const updatedItems = formData.items.map(item => {
                                  if (item.productType === 'chicken' && item.chickenNature === 'ready-to-cook' && item.cutUp === 'yes') {
                                    return { ...item, cutPieces: pieces };
                                  }
                                  return item;
                                });
                                onChange({ ...formData, items: updatedItems });
                              }}
                              className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg text-sm focus:border-yellow-400 focus:outline-none"
                              placeholder="Enter number of pieces"
                            />
                            <div className="text-xs text-gray-500 mt-1">
                              Fee: {getCutUpFee(formData.items.find(item => item.productType === 'chicken' && item.chickenNature === 'ready-to-cook' && item.cutUp === 'yes')?.cutPieces || '6')} CFA per chicken
                              {parseInt(formData.items.find(item => item.productType === 'chicken' && item.chickenNature === 'ready-to-cook' && item.cutUp === 'yes')?.cutPieces || '6') > 4 ? ' (>4 pieces)' : ' (≤4 pieces)'}
                            </div>
                          </div>
                        )}
                      </div>
