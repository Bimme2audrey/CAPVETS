...
export default function MediaManagerPage() {
  ...
  return (
    <div>
      ...
      {/* Upload Form */}
      <form onSubmit={handleUpload} className="bg-white rounded-xl border border-gray-200 p-5 mb-6 flex flex-wrap gap-4 items-end">
        <div className="flex-1 min-w-[200px]">
          <label className="block text-sm font-semibold text-gray-700 mb-1.5">File</label>
          <input type="file" accept="image/*,video/*" className="w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100" />
        </div>
        <div className="flex-1 min-w-[200px]">
          <label className="block text-sm font-semibold text-gray-700 mb-1.5">Description</label>
          <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="e.g., Small chicken 1.5kg" className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg text-sm focus:border-yellow-400 focus:outline-none" />
        </div>
        <button type="submit" disabled={uploading} className="px-6 py-2.5 bg-green-700 text-yellow-400 rounded-lg font-semibold text-sm hover:bg-green-800 transition-colors disabled:bg-gray-300 disabled:text-gray-500 cursor-pointer">
